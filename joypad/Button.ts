...
module Gamepads {
    ...
    export class Button extends Phaser.Plugin {
        ...
        enableCooldown(seconds:number) {
            ...
            this.sprite.mask = new PieMask(this.game, mask_radius, mask_x, mask_y);
        }

        disableCooldown() {
            this.cooldown.enabled = false;
            (<PieMask> this.sprite.mask).drawCircleAtSelf();
            (<PieMask> this.sprite.mask).atRest = true;
        }
        ...
        update() {
            ...
            if (this.cooldown.enabled) {
                ...
                if (elapsed > cooldown) {
                    ...
                    if (!(<PieMask> this.sprite.mask).atRest) {
                        (<PieMask> this.sprite.mask).drawCircleAtSelf();
                        (<PieMask> this.sprite.mask).atRest = true;
                    }
                ...
                (<PieMask> this.sprite.mask).drawWithFill(pj, 0xFFFFFF, 1);
                (<PieMask> this.sprite.mask).atRest = false;
